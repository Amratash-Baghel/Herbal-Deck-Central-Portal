...
import { canGroup, isNearBottom, mergeMessages, insertAt, isGifUrl, PICKER_EMOJI } from "../chat-model";
            ...
            return <Fragment key={m.id}>{newDay && <div className="cp-date"><span>{day(m.created_at)}</span></div>}<article tabIndex={-1} onPointerDown={event => {
              ...
              <div className="cp-message-content">{first && !mine && <div className="cp-sender">{name(m.sender_id)}</div>}
                <div className="cp-bubble" style={!mine ? { "--sender": person(m.sender_id)?.color || "var(--primary)" } as CSSProperties : undefined}>
                  ...
                  {m.deleted ? <p className="cp-deleted">This message was deleted</p> : <><p className="cp-body">{m.body.split(/(https?:\/\/[^\s]+)/g).map((part, j) => /^https?:\/\//.test(part) ? <a key={j} href={part} target="_blank" rel="noreferrer">{isGifUrl(part) ? <img className="cp-gif-inline" src={part} alt="GIF" loading="lazy"/> : part.length > 90 ? `${part.slice(0, 87)}…` : part}</a> : part)}</p>{m.attachments?.length > 0 && <MessageAttachments supabase={supabase} attachments={m.attachments}/>} {m.files && localFiles(m.files)}{detectShareLinks(m.body).length > 0 && <LinkPreviewCards links={detectShareLinks(m.body)}/>}</>}
                </div>
